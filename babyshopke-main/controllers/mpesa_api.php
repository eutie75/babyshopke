 ...
 *
 * POST /babyshopke/backend/controllers/mpesa_api.php
...
// ── CORS for local Vite dev (adjust origin in production) ───────────────────
...
if (in_array($origin, $allowed, true)) {
    ...
    // Allow same-origin or direct requests (e.g. from XAMPP pages)
...
if ($action === 'initiate') {
    ...
    $cartItems     = $body['cart_items'] ?? [];   // [{ id, name, price, qty }]
    ...
    // Normalise M-Pesa phone to 2547XXXXXXXX
    try {
    ...
    // In production you'd validate cart_items against DB prices.
    // Here we trust the frontend totals (add server-side validation for real launch).
    $subtotal = 0.0;
    ...
    // ── Create the order ─────────────────────────────────────────────────────
    // We create it in 'pending' state; MpesaController::stkPush will flip it to 'awaiting_payment'.
    try {
        $db->beginTransaction();
        ...
        ');
        // Use guest user_id = 0 when not logged in; adjust once you wire up session auth
        ...
        // Insert order items (snapshot of prices at time of order)
        $itemStmt = $db->prepare('
